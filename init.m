...
nEnd = [0 0 0];
TimeStep = 500;       % seconds , time for which the system has to stay  steady

% Definitions of general parameters
TimeTransition = TimeStep-100;
t = 0:0.1:TimeTransition; % Vector of discrete time instants [s]

epsilon_L = 0.05; % Min. frac. of L to be trav. at max. (min.) vel.
theta_a = 0.90; % Function switching threshold (0.6 <= theta_a < 1)
theta_d = 0.15; % Function switching threshold (0 < theta_d <= 0.4)
theta_0 = exp(-10); %Fcn. switching thr.(exp(??13)<=theta_0<=exp(??7))
...
S = sim('part1','StartTime',"0.0",'StopTime',"3500",'SimulationMode','normal');
%sim('part1')
